...
const updateProfile = async (user: IRequestUser, payload: IUpdatePatientProfilePayload) => {

    // first find the if patient is exist or not
    ...
    await prisma.$transaction(async (tx) => {
        // if payload.patientInfo is there....
            ...
            for (const report of payload.medicalReports) {
                if (report.shouldDelete && report.reportId) {
                    ...
                    if (deleteReport.reportLink) {
                        await deleteFromCloudinary(deleteReport.reportLink);
                    }
